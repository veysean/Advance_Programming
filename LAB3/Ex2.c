...
/*create an array integer which has 100 elements.  Then manage it as the following menu:
1) add : ask user to input an integer value to store in array, add only one element for once
and start from index 0.
2) Print the array to screen: print only the element which user added
3) Sort the value of the array from smallest to the largest: sort only the element which user
added.
4) Tell user to input a number to search from sorted array*/
